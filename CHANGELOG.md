...
- Run results survive a reload. Each row's verdict, timings and captured output, and the header's pass/fail tint, are stored by the host in workspace state keyed by problem id and sample number, so reopening the window paints the list back exactly as the last run left it. A re-import that drops a sample drops its result with it, and a blob over 4 MB is skipped rather than written.
- Submit sends the file the problem is linked to, not whatever sits in the editor. A problem with no linked file greys its Submit button out and says to press Run in its header first; right-clicking the source chip in a header unlinks the file again and clears that problem's tint.
- Running one sample clears the rest of that problem's results first, so the header tint and the row badges describe that sample alone. A single sample leaves no `n/m` count: one case is not a verdict for the problem.
...
- A submit tab is reloaded once the token it carries has been spent. Codeforces takes an anti-bot token exactly once and the widget goes on showing the used one, so the second submit driven from a parked tab - problem B right after problem A, both on the contest's one submit page - was replayed with a dead token and came back as "Please complete the anti-bot verification". A tab that has posted now reloads before it is replayed again; a tab that has not still keeps its page, so a token the user cleared by hand after "clear it there, then submit again" survives. The load is waited out on the tab's own load event rather than polled for, since polling cannot tell a page that is about to be replaced from the one already there.
- A Codeforces refusal of an unchanged source names the verdict that source already got. Codeforces takes a source once per problem, so pressing Submit again on a file that has not been edited comes back as "You have submitted exactly the same code before" - which says nothing about how the submission it duplicates went. The submit now reads that problem's newest row and reports it as `Codeforces already has this exact source for B (Accepted)`, so a repeat press answers the question instead of sending the user to the browser.
- `cp-helper.submitNotifications` picks what a submit pops when it settles or fails: `auto` (the default), `sticky` or `off`. A notification raised the plain way is VS Code's to drop, on a timer that only runs while the window has focus - so a verdict raised while the judge is in front used to sit there until VS Code was looked at again, one per submit, and nothing could close it early. `auto` shows a verdict with nothing wrong in it for 1s and closes it, and hands a failure to VS Code at information severity - only VS Code's own timer holds a notification open while the pointer is over it and files it in the notification centre afterwards, and information is the shortest that timer goes, 10s against the 15s an error would take; `sticky` leaves it to VS Code as before; `off` leaves it to the problem header and the output log, which carry the text in every mode.
- `cp-helper.notifications` does the same for every other notification - imports, exports, stress runs, and the failures behind Run, Debug and Export - so nothing CP Helper says has to be clicked away. Every message it silences is written to the output log first, and prompts that ask a question (the submit confirmation, the stress "Add to Samples" offer) are left alone: a question has to stay until it is answered.
