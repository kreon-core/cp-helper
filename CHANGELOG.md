...
- `cp-helper.submitNotifications` picks what a submit pops when it settles or fails: `auto` (the default), `sticky` or `off`. VS Code dismisses an information toast by itself but keeps a warning or an error until it is clicked away, so a wrong answer and every failed submit used to pile up notifications that had to be cleared by hand. `auto` says the same things as a self-dismissing toast, `sticky` is the old behaviour, and `off` leaves it to the problem header and the output log, which carry the text in every mode.
