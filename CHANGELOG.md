...
- `cp-helper.submitNotifications` picks what a submit pops when it settles or fails: `auto` (the default), `status`, `sticky` or `off`. A toast is dropped on a timer VS Code owns - 10s for information, 12s for a warning, 15s for an error - and that timer only runs while the window has focus, so a verdict raised while the judge is in front used to sit there until VS Code was looked at again, one per submit. `auto` says everything at information level, for the shortest of those timers; `status` skips the toast for a status bar message that clears itself after 2s, or 4s when it reports a problem, whether the window is focused or not; `sticky` is the old error / warning notification; and `off` leaves it to the problem header and the output log, which carry the text in every mode.
